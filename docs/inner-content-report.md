...
Final Gallery polish: two further regression tests first reported 7 passing and 2 failing. The known empty 1512×193 legacy footer spacer is now removed by the generator wherever it occurs (Gallery, Hospital Outreach, Marriage Seminars and Leadership Seminars). Gallery's archive links sit inside `.page-content`, retaining the full-bleed slideshow. The slideshow's reduced-motion flag also accepts `?motion=off` alongside the operating-system preference, so the review option disables automatic advancement. Fresh full-suite result: **30 passing, 0 failing**; whitespace check clean.
